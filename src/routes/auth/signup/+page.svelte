		...
		try {
			...
			if (result.error) {
				...
				if (errorMessage.toLowerCase().includes('already') || errorMessage.toLowerCase().includes('exists')) {
					...
					error = errorMessage || 'Failed to sign up. Please try again.';
				}
				isLoading = false;
			} else {
	...
	<div class="flex w-full max-w-sm flex-col gap-6">
		...
		<div class="flex flex-col gap-6">
			<div class="text-center">
				...
				<p class="text-muted-foreground">Get started with Acme Inc.</p>
			</div>
			...
			<p class="text-center text-sm text-muted-foreground">
				Already have an account? <a href="/auth/login" class="underline-offset-4 hover:underline">Sign in</a>
			</p>
		</div>
	</div>
	<p class="px-6 text-center text-xs text-muted-foreground">
		By creating an account, you agree to our <a href="/terms" class="underline-offset-4 hover:underline">Terms of Service</a>
